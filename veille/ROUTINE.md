...
Chaque numéro a 6 images réelles : couverture, image d'inspiration, 4 signaux. Jamais d'image générée.
...
5. Si une image arrive dans un format illisible (SVG enveloppant une image, par exemple), extraire l'image intégrée avec Pillow.
6. Relever le HEX de `couleur` sur l'une de ces images (moyenne d'une zone typique avec Pillow).
