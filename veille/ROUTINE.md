...
- Un **signal faible** (`signal_faible`), **au format note, court** : quelque chose de petit, de précoce ou d'étrange, encore peu visible, mais relié à un mouvement de fond (une recherche, une pratique marginale, une niche, un usage détourné, une initiative locale). Jamais une tendance déjà installée. Champs : `lien` (une phrase de transition qui rattache le signal à l'édito ou à la tendance), `titre`, `lieu`, `texte` (**40 à 60 mots**, le fait précis), `pourquoi` (une phrase courte : pourquoi ça pourrait compter demain), `pour_funfun` (une phrase, qui commence en minuscule car elle suit « Pour FunFun : »), `sources`.
